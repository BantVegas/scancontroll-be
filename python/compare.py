...
def split_labels_grid_with_gaps(scan_img, rows, cols, label_w, label_h, gap_x, gap_y):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            # Ak crop nie je presného rozmeru, resize
...
def main():
    if len(sys.argv) != 10:
        print("Použitie: ai_compare_grid.py <master_path> <scan_path> <output_dir> <rows> <cols> <label_h_px> <label_w_px> <gap_x_px> <gap_y_px>")
        print("Príklad: ai_compare_grid.py master.png scan.png out_dir 3 4 320 220 20 18")
        sys.exit(1)

    master_path, scan_path, output_dir, rows_s, cols_s, label_h_s, label_w_s, gap_x_s, gap_y_s = sys.argv[1:]
    rows = int(rows_s)
    cols = int(cols_s)
    label_h = int(label_h_s)
    label_w = int(label_w_s)
    gap_x = int(gap_x_s)
    gap_y = int(gap_y_s)
        ...
        if bboxes:
            for (x,y,w,h) in bboxes:
                ...
                errors.append({
                    ...
                    "col": c,
                    "type": "Deviation",
                    "desc": f"Modrá čmáranica na etikete r{r}c{c}",
    ...
    print(f"Uložil som output.png a errors.json do {output_dir}")
